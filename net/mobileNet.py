...
def MobileNet(input_shape=[224,224,3],depth_multiplier=1,dropout=1e-3,classes=1000):
    ...
    # 7,7,1024 -> 1,1,1024
    x = GlobalAveragePooling2D()(x)#GAP拉直
...
def _depthwise_conv_block(inputs, pointwise_conv_filters,depth_multiplier=1, strides=(1, 1), block_id=1):
    x = DepthwiseConv2D((3, 3),padding='same', depth_multiplier=depth_multiplier,strides=strides,use_bias=False,name='conv_dw_%d' % block_id)(inputs)#进行深度卷积
    x = BatchNormalization(name='conv_dw_%d_bn' % block_id)(x)#BN层
    x = Activation(relu6, name='conv_dw_%d_relu' % block_id)(x)#激活层
    x = Conv2D(pointwise_conv_filters, (1, 1),padding='same',use_bias=False,strides=(1, 1),name='conv_pw_%d' % block_id)(x)#1*1卷积核进行通道调整
    x = BatchNormalization(name='conv_pw_%d_bn' % block_id)(x)#BN层
    return Activation(relu6, name='conv_pw_%d_relu' % block_id)(x)#激活层
